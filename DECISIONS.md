...
- **Data folders created by default:** `app_data_path` is the data root (default `/data`) and the playbooks create `/data`, `/data/jpg`, `/data/video`, `/data/jpg/no_hook`, `/data/jpg/no_overlay`, `/data/ad_hoc`, `/data/ad_hoc/jpg`, and `/data/ad_hoc/video` in the normal run. No longer optional.
...
- **NetworkManager + netplan + WiFi (2.4/5) pattern:** Networking uses NetworkManager as netplan renderer (not systemd-networkd). Install NetworkManager; write **01-network-manager.yaml** (renderer only) and **99-machine-network.yaml** (ethernets); apply netplan. WiFi: create two NM profiles (2.4 GHz band bg priority 100, 5 GHz band a priority 10); run idempotent **modify** every time to keep settings aligned; then attempt preferred profile activation in a non-fatal way even when SSID is absent. Default SSID OFFICEGST; playbook succeeds if SSID is not yet available; autoconnect takes over when network appears. `machine_wifi_interface` may be pinned (for example `wls3`) or auto-detected when unset. Do not restart NetworkManager during SSH-only bootstrap.
