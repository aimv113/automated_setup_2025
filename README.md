...
After reboot, run the verification playbook to confirm everything is working. The verification playbook also configures **networking** via **NetworkManager** + netplan (DHCP + camera static; optional WiFi) and **timezone** (America/Chicago). Machine setup is complete after that. For king_detector and crane display, run the setup script in the king_detector repo (see that repo’s admin/SETUP.md).
